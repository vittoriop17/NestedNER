...
class DecoderLSTM(nn.Module):
    ...
    def __init__(self, no_of_output_symbols, embedding_size=16, hidden_size=25, use_attention=True,
                 display_attention=False, device='cpu', use_attention_window=True, window_size=50, slide=10):
        ...
        self.use_attention_window = True
    ...
    def _compute_context(self, encoder_hidden_states, last_state, idx):
        left_lim = max(0, (idx * self.slide) - int(self.window_size / 2)) if self.use_attention_window else 0
        right_lim = min(encoder_hidden_states.shape[1], (idx * self.slide) + int(self.window_size / 2)) if self.use_attention else encoder_hidden_states.shape[1]
